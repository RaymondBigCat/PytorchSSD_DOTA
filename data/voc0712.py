...
class DOTADetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, preproc=None, target_transform=None,
        ...
        self.catNms = catNms
        '''
        # %s 通配符，用于匹配传入的tuple，构造str
        self._annopath = os.path.join('%s', 'Annotations', '%s.xml')
        self._imgpath = os.path.join('%s', 'JPEGImages', '%s.jpg')
        self.ids = list()
        
        # 构造数据集的路径
        # (year,name) = [('2007', 'trainval'), ('2012', 'trainval')]
        for (year, name) in image_sets:
            self._year = year
            
            # 构造数据集的root路径 （VOC1007,VOC2012）
            rootpath = os.path.join(self.root, 'VOC' + year)
            
            # Image/Main里存放的内容：按包含的目标类别，存放图片id的.txt文件
            # e.g. aeroplane_train.txt存放的是所有包含飞机的图片的id
            # 遍历这些图片的id，将其存入一个list （后续用来构造每个图片/label的路径）
            for line in open(os.path.join(rootpath, 'ImageSets', 'Main', name + '.txt')):
                self.ids.append((rootpath, line.strip()))
        '''
    ...
    def __getitem__(self, index):
        ...
        # 通过 % 构造图片/标注文件的路径
        # 此时target是一个Element对象，在Annotationtransform里被进一步解析
        target = ET.parse(self._annopath % img_id).getroot()
        img = cv2.imread(self._imgpath % img_id, cv2.IMREAD_COLOR)
        height, width, _ = img.shape

        if self.target_transform is not None:
            # target_transform 进一步解析XML标注
            target = self.target_transform(target)

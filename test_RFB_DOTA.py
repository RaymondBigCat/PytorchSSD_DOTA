...
from data import VOCroot,COCOroot,VOCroot_eval
...
parser = argparse.ArgumentParser(description='RFB Net for DOTA')
...
parser.add_argument('-d', '--dataset', default='DOTA',
                    help='VOC or COCO version (Default is DOTA)')
...
def test_net(save_folder, net, detector, cuda, testset, transform, max_per_image=300, thresh=0.005):
    ...
    for i in range(num_images):
        ...
        boxes = boxes[0]#原因是每次都只是一张图，所以此句由[1, num_priors, 5]转为[num_priors, 5]
        scores=scores[0]
...
if __name__ == '__main__':
    ...
    num_classes = args.num_classes
    net = build_net(img_dim, num_classes,test=True)    # initialize detector
    ...
    print(net)
    # load data
    testnet = DOTADetection(DOTAroot,
                            image_sets='valsplit',
                            target_transform=DotaAnnTrans(),
                            catNms=['plane', 'small-vehicle', 'large-vehicle', 'ship'])
    if args.cuda:
        net = net.cuda()
